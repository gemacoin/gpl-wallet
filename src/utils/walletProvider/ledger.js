...
import {
  getPublicKey,
  gemachain_derivation_path,
  gemachain_ledger_sign_bytes,
  gemachain_ledger_sign_transaction,
  gemachain_ledger_confirm_public_key,
} from './ledger-core';
...
export class LedgerWalletProvider {
  constructor(args) {
    ...
    this.solanaDerivationPath = gemachain_derivation_path(
      this.account,
      this.change,
      this.derivationPath,
    );
  ...
  signTransaction = async (transaction) => {
    const sig_bytes = await gemachain_ledger_sign_transaction(
      this.transport,
  ...
  createSignature = async (message) => {
    const sig_bytes = await gemachain_ledger_sign_bytes(
      this.transport,
      this.solanaDerivationPath,
      message,
    );
  ...
  confirmPublicKey = async () => {
    return await gemachain_ledger_confirm_public_key(
      this.transport,
      this.solanaDerivationPath,
    );
  };

...
import { useConnection, useGemachainExplorerUrlSuffix } from '../utils/connection';
    ...
    instructions.forEach((instruction) => {
      if (!instruction) {
        ...
        if (instruction.type === 'raydium') {
          ...
          // New GPL token accounts are only considered safe if they are owned by this wallet and newly created
...
export default function SignTransactionFormContent({
  ...
  const explorerUrlSuffix = useGemachainExplorerUrlSuffix();
